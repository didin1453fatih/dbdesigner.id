...
import LoadProject from "@/request/Project/open.request";
import SaveProject from "@/request/Project/save.request";
import { request2, requestHelper } from "@/helper/RequestConnector";
export default {
  // setClipboard: request2(async context => {
  //   /**
  //    * action : 'copy' | 'cut'
  //    * type   : 'coloumn' | 'table'
  //    */
  ...
  //     type: context.inputs.type,
  //     data: context.inputs.data
  //   })
  // }),
  // getClipboard: request2(async context => {
  // }),
  ...
  loadProject: request2(async context => {
    ...
    try {
      ...
      var path = "?action=open&uuid=" + respond.payload.id;
