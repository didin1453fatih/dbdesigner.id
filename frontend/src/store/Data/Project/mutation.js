...
export default {
  ...
  UPDATE_COLOUMN_POSITION(state, raw) {

    var table_id = raw.table_id
    var newKeyColoumnOrder = raw.newKeyColoumnOrder
    var tmpColoumns = {}

    for (let i = 0; i < newKeyColoumnOrder.length; i++) {
      const key = newKeyColoumnOrder[i];
      tmpColoumns[key] = state.dataDiagramNew[table_id].coloumns[key]
    }

    state.dataDiagramNew[table_id].coloumns = tmpColoumns
